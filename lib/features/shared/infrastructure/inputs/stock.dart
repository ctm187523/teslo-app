...
class Stock extends FormzInput<int, StockError> {
  ...
  StockError? validator(int value) {
    ...
    //probamos de parsear si es Integer el numero obtenido, si no devolvemos -1
    final isInteger = int.tryParse( value.toString()) ?? -1;
    if (isInteger == -1) return StockError.format; //no logro parsearlo y manda error
